  ...
  <div>
    ...
    <div class="table">
      ...
      <el-table :data="paginatedReserveInfoList" stripe>
        ...
        <el-table-column label="操作" width="120" align="center">
          <template v-slot="scope">
            <el-button plain type="danger" size="mini" @click="seeCancel(scope.row)" :disabled="scope.row.approvalStatus!='正常'">申请取消</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    getReserveInfoListByDate(date) {
      this.reserveInfoListByDate = this.reserveInfoListAll.filter((order) => {
        ...
        const orderDateString = this.formatDate(order.clinicTime).split(
          "星"
        )[0];
